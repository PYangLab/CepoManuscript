---
title: "AUC heatmap of simulation results (non-DS methods)"
...
# Generate heatmap
```{r}
sim_dx_result_mat <- rbind(matrix(nrow=7, ncol=7, prob.Dx[["Voom"]]), 
                           matrix(nrow=7, ncol=7, prob.Dx[["ttest"]]),
                           matrix(nrow=7, ncol=7, prob.Dx[["Limma"]]), 
                           matrix(nrow=7, ncol=7, prob.Dx[["MAST"]]),
                           matrix(nrow=7, ncol=7, prob.Dx[["Wilcoxon"]])
)
...
annot_row <- data.frame(
  method=rep(c("Voom","ttest","Limma","MAST", "Wilcoxon"), each=7),
  sd.scale=rep(seq(1,3.4,by=0.4), 5)
)
...
pheatmap(sim_dx_result_mat, cluster_rows = F, 
  ...
  ggsave(filename ="../../../Figures/simulation/simulation_AUCROC_heatmap_DE.pdf", width=9, height=20)
```
