...
segIndex <- function(mat){
    ...
    segMat <- DelayedArray::cbind(nz, cvs)
    rownames(segMat) <- rownames(mat)
    
    segMat_rank <- DelayedArray::t(DelayedMatrixStats::colRanks(segMat))
    colnames(segMat_rank) <- c("nz", "cv")
    
    x1 <- segMat_rank[,"nz"]/(nrow(segMat_rank)+1)
    x2 <- 1 - segMat_rank[,"cv"]/(nrow(segMat_rank)+1)
